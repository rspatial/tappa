...
from tappa.time import setTime, getTime, has_time, timeInfo


def test_time_roundtrip():
    """Setting a datetime and retrieving it preserves the value."""
    r = rast(nrows=2, ncols=2, names="random")
    r = setValues(r, [1.0, 2.0, 3.0, 4.0])
    dt = datetime(2000, 1, 2, 12, 12, 12, tzinfo=timezone.utc)
    r_timed = setTime(r, [dt])
    assert has_time(r_timed)
    t_back = getTime(r_timed)
    assert len(t_back) == 1
    assert r_timed.time() == t_back
    assert getTime(r_timed, format="raw") == list(r_timed.time_raw)


def test_time_info_zone():
    """timeInfo() returns timezone information."""
    r = rast(nrows=2, ncols=2)
    r = setValues(r, [1.0, 2.0, 3.0, 4.0])
    dt = datetime(2000, 1, 2, 12, 12, 12, tzinfo=timezone.utc)
    r_timed = setTime(r, [dt], tz="UTC")
    info = timeInfo(r_timed)
