...
Uses :func:`terra.distance.distanceXY`, which follows ``R/distance.R``:
``distance(matrix, y=missing, lonlat=..., method="geo", ...)``.
"""
import math
import numpy as np
from tappa.distance import distanceXY
...
def test_distance_planar_sequential():
    ...
    d = distanceXY(coords(), lonlat=False, sequential=True, method="geo")
...
def test_distance_planar_pairwise():
    ...
    d = distanceXY(coords(), lonlat=False, sequential=False, method="geo")
...
def test_distance_lonlat_sequential():
    ...
    d = distanceXY(coords(), lonlat=True, sequential=True, method="geo")
...
def test_distance_lonlat_pairwise():
    ...
    d = distanceXY(coords(), lonlat=True, sequential=False, method="geo")
